$(document).ready(function () {
    ...
    var myDataUrl = `${solrUrl}/select?q=pageType%3ABlog&omitHeader=true&rows=1000&fq=siteName_s:` + siteName;
	...
	if(urlParams != null) {
		//$('select[name=tagValue]').val(cat).trigger('change');
		//$('select[name=subTagValue]').val(subcat).trigger('change');
		...
		let param1 = (cat != '') ? `&fq=tagValue:"${cat}"` : '';
       	...
       	$('.row.blog-grid').html('');
       	myDataUrl = `${solrUrl}/select?q=pageType%3ABlog${params}&omitHeader=true&rows=100&fq=siteName_s:` + siteName;
       ...
       $.ajax({
         ...
         success: function (searchresponse) {
           ...
           if (isUpdateFilters == true) {
             ...
             // Empty the options
             $('.blogmaincategory, .blogsubcategory').find('option').remove().end().append('').val('');
             ...
             let categoryArr = [];
               ...
               generateOptions(categoryArr, '.blogmaincategory.selectpicker');
               generateOptions(subCatergoryArr, '.blogsubcategory.selectpicker');
           ...
           <div class="blog-card">
            <a href="${item.url}" class="link-card get-ranks" title="" data-isb-search-info='{"rank":"${rankCount++}", "category": "${item.subTagValue ? 'tag' : ''}", "resultType": "${item.title ? item.title : ''}", "linkUrl":"${item.url}"}'></a>
             ...
             <div class="news-ctnt">
               ...
               <p class="font-14">${item.publishedDate ? new Date(item.publishedDate).getDate() : ''} ${item.publishedDate ? monthNames[new Date(item.publishedDate).getMonth()] : ''} ${item.publishedDate ? new Date(item.publishedDate).getFullYear() : ''}</p>
             </div>
           </div>
         </div>`).join('')}`;
